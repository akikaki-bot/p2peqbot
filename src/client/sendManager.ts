...
export class ChannelSendManager {
    ...
    async buildWithImage() {
        const _data = this.data;
        if( !_data.imageBuffer ) throw new Error('buildWithImage() Required Buffer data.');

        const image = new AttachmentBuilder( _data.imageBuffer ).setName('areaquake.png')
        const embed = new EmbedBuilder().setTitle( _data.title ?? "不明な情報種別" )
                                        .setDescription( _data.description ?? "不明な情報説明" )
                                        .setFooter({ text : `EarthQuakeBot | ${typeof _data.footerText !== "undefined" ? _data.footerText : "AreaPage"} - ${_data.page ?? 1}/${_data.maxPage ?? 1}` })
                                        .setColor( _data.color ?? "Default" )
                                        .setImage("attachment://areaquake.png")
        
        await this.sendWithAttachment( embed , image )
    }

    async sendWithAttachment( embed : EmbedBuilder , image : AttachmentBuilder ) {
